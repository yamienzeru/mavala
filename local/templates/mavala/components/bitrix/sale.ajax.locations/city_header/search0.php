...
if(CModule::IncludeModule("sale"))
{
	if(!empty($_REQUEST["search"]) && is_string($_REQUEST["search"]))
	{
		...
		if($set_search)
		{
			...
			while ($arCity = $rsCityList->GetNext())
				$arCities[] = $arCity;
			...
			if(is_array($kladr_search["result"]) && count($kladr_search["result"]))
				foreach($kladr_search["result"] as $arKLADRCity)
				{
					...
					foreach($arCities as $arCity)
						if($arCity["CITY_NAME_ORIG"] == $arKLADRCity["full_name"] && $arCity["REGION_NAME_ORIG"] == $arKLADRCity["parents"][0]["full_name"])
						{
							//такой город есть
							$city_id = $arCity["ID"];
							break;
					...
					if(!$city_id)
						$arKLADRCities[] = $arKLADRCity;
				}

			if(!empty($arKLADRCities))
			{
				$arRegions = array();
				...
				while ($arRegion = $dbRegion->Fetch())
					$arRegions[] = $arRegion;
				/*print_p($arRegions);
				print_p($arKLADRCities);*/
				foreach($arKLADRCities as $arKLADRCity)
				{
					$region_id = 0;
					if(strlen($arKLADRCity["parents"][0]["full_name"]))
					{
						...
						if(!$region_id)
						{
							...
							/*print_p($arKLADRCity["full_name"]);
							print_p($arRegionFields);*/
